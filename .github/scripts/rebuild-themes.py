...
import yaml
from theme_check import widget_names

THEMES_FOLDER = "./themes"
THEMES_DATA_FILE = "./themes.json"


def read_widgets(folder):
    try:
        with open(os.path.join(folder, "config.yaml"), encoding="utf-8") as f:
            return widget_names(yaml.safe_load(f))
...
def get_publish_date_from_theme(folder):
    ...
    if os.path.exists(theme_data_file):
        try:
            with open(theme_data_file) as f:
...
def main():
    ...
    if os.path.exists(THEMES_DATA_FILE):
        try:
            with open(THEMES_DATA_FILE) as f:
    ...
    theme_folders = [
        os.path.join(THEMES_FOLDER, theme)
        for theme in sorted(os.listdir(THEMES_FOLDER))
        if os.path.isdir(os.path.join(THEMES_FOLDER, theme))
    ]
    ...
    for theme_folder in sorted_theme_folders:
        ...
        theme_data["widgets"] = read_widgets(theme_folder)
